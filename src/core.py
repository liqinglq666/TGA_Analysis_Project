...
def _interp_value(df: pd.DataFrame, column: str, temperature: float) -> float:
    ...
    temp_min, temp_max = clean["Temp"].min(), clean["Temp"].max()
...
def _calculate_mass_loss(
    ...
    """Return mass loss in percentage units under the selected reference basis.

    as_input keeps the old behavior for TG curves already exported as percent mass.
    normalize_105 and normalize_600 convert absolute mass or percent mass to wt.%
    relative to the selected reference temperature.
    """
...
def calculate_ch_content(
    ...
    total_loss = _calculate_mass_loss(tg_df, t_start, t_end, ref_mode=ref_mode)
    avg_bg_rate = (abs(dtg_s) + abs(dtg_e)) / 2.0
    bg_loss_raw = (avg_bg_rate / heating_rate) * (t_end - t_start) if heating_rate > 0 else 0.0
...
def calculate_carbonation(
    ...
    loss_co2 = _calculate_mass_loss(tg_df, search_range[0], search_range[1], ref_mode=ref_mode, strict=strict)
    return max(0.0, loss_co2 * FACTOR_CACO3)
...
def calculate_friedels_salt(
    ...
    loss_h2o = _calculate_mass_loss(tg_df, search_range[0], search_range[1], ref_mode=ref_mode, strict=strict)
    return max(0.0, loss_h2o * FACTOR_FS)
...
def calculate_csh_estimation(
    ...
    return max(0.0, _calculate_mass_loss(tg_df, search_range[0], search_range[1], ref_mode=ref_mode, strict=strict))
...
def calculate_sample_summary(
    ...
    """Calculate all standard TGA indicators for one sample once.

    This helper keeps GUI export / clipboard / batch workflows consistent and avoids
    calling the same CH calculation repeatedly for one sample.
    """
    ...
    return {
        ...
        "CH Total (%)": ch_result["ch_traditional"] if ch_result else 0.0,
        "CH Net (%)": ch_result["ch_corrected"] if ch_result else 0.0,
        "CH Background (%)": ch_result["bg_loss"] if ch_result else 0.0,
        "Wn (%)": calculate_bound_water(
            tg_df, co2_loss, mode=bound_water_mode, ref_mode=ref_mode, strict=strict
        ),
        "CO2 Loss (%)": co2_loss,
